...
export default function WebsiteQRCodeGenerator({ website, onClose }: WebsiteQRCodeGeneratorProps) {
    const [title, setTitle] = useState(website.name || 'Project Portal');
    const [description, setDescription] = useState('Scan to verify details and view property inventory');
    const [qrSize, setQrSize] = useState(250);

    const baseUrl = typeof window !== 'undefined' ? window.location.origin : '';

    // Determine the best URL to share
    const targetUrl = website.customDomain
    ...
    const qrCodeUrl = `https://api.qrserver.com/v1/create-qr-code/?size=${qrSize}x${qrSize}&data=${encodeURIComponent(targetUrl)}`;
    ...
    const handleDownload = async () => {
        try {
            ...
            link.href = url;
            link.download = `qr-${website.slug}.png`;
        ...
        <div className="modal-backdrop d-flex align-items-center justify-content-center" style={{ backgroundColor: 'rgba(0,0,0,0.7)', zIndex: 1100, position: 'fixed', top: 0, left: 0, right: 0, bottom: 0 }}>
            <div className="modal-content bg-white p-4 rounded-4 shadow-lg animate-scale-in" style={{ maxWidth: '500px', width: '90%' }}>
                <div className="d-flex justify-content-between align-items-center mb-4">
                    <h5 className="fw-bold mb-0">Website QR Code</h5>
                ...
                <div className="text-center mb-4 py-3 bg-light rounded-4">
                    <div className="qr-container bg-white p-3 d-inline-block rounded-3 shadow-sm border">
                        <img src={qrCodeUrl} alt="Website QR Code" style={{ width: '100%', height: 'auto', maxWidth: '250px' }} />
                    </div>
                    <div className="mt-3">
                        <span className="badge bg-primary-subtle text-primary px-3 rounded-4 text-truncate" style={{ maxWidth: '100%' }}>
                ...
                <div className="row g-3 mb-4">
                    ...
                    <div className="col-md-6">
                        ...
                        >
                            ...
                            <option value={1000}>High Res (1000px)</option>
                        </select>
                    </div>
                </div>
