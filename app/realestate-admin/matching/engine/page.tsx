...
function MatchingContent() {
    ...
    useEffect(() => {
        const saved = localStorage.getItem('propMatchAdmin_hideGuide');
        if (saved === 'true') {
    ...
    const toggleGuide = (show: boolean) => {
        ...
        localStorage.setItem('propMatchAdmin_hideGuide', (!show).toString());
    };
    ...
    const fetchLeads = useCallback(async (showRefreshToast = false) => {
        ...
        try {
            const [waitingRes, matchedRes] = await Promise.all([
                automationApi.getWaitingLeads(),
                automationApi.getMatchedLeads()
            ]);
            ...
            if (showRefreshToast) {
                setToast({ show: true, message: 'Global Matching Engine refreshed.', type: 'info' });
            }
    ...
    const handleForceMatch = async (leadId: string, tenantId: string) => {
        setMatchingLeadId(leadId);
        setPreviewLead(null);
        try {
            const res = await automationApi.forceMatch(leadId, tenantId);
            if (res.success) {
    ...
    const handlePreviewMatch = async (lead: any) => {
        ...
        try {
            const res = await automationApi.previewMatch(lead.id, lead.tenantId);
            if (res.success) {
                if (res.data.matchCount > 0) {
                    setPreviewLead(lead);
    ...
    const handleViewLead = (leadId: string) => {
        window.location.href = `/realestate-admin/leads?id=${leadId}`;
    };
            ...
            <div className="container-fluid py-4">
                <div className="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        ...
                        <p className="text-muted mb-0">Platform-wide monitor for high-intent leads waiting for property matches.</p>
                    </div>
                    <div className="d-flex gap-2">
                        {!showHowItWorks && (
                            <button className="btn btn-light btn-sm rounded-pill px-3 fw-bold text-primary shadow-sm border" onClick={() => toggleGuide(true)}>
                                <i className="bi bi-info-circle me-1"></i> Show Guide
                            </button>
                        )}
                        ...
                        <div className="card-body p-4 p-lg-5">
                            <div className="row align-items-center">
                                <div className="col-lg-8">
                                    <h3 className="fw-bold mb-3">How PropMatch™ Sells Across the Platform</h3>
                                    <p className="opacity-75 mb-4">Our AI Matching Engine works 24/7 for every agency on the platform. Here is how the automated workflow helps your tenants:</p>
                                        ...
                                        <div className="col-md-6">
                                            <div className="d-flex gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="small opacity-75">AI extracts budget/location preferences from incoming lead messages.</div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="col-md-6">
                                            <div className="d-flex gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="small opacity-75">Global scan runs every time a new property is listed, matching it to waiting leads.</div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="col-md-6">
                                            <div className="d-flex gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="small opacity-75">PropMatch™ includes high-value properties within 10% of the target budget.</div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="col-md-6">
                                            <div className="d-flex gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="small opacity-75">Automated personalized WhatsApp & Email notifications are sent instantly.</div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div className="col-lg-12">
                        <div className="card border-0 shadow-sm rounded-4 overflow-hidden mb-4">
                            <div className="card-header bg-white py-3 border-0 d-flex gap-3 px-4">
                                ...
                                >
                                    Global Waiting Queue ({waitingLeads.length})
                                </button>
                                <button
                                    ...
                                    style={{ borderRadius: '20px', padding: '0.4rem 1rem' }}
                                >
                                    Total Successful Matches ({matchedLeads.length})
                                </button>
                            </div>
                                            ...
                                            waitingLeads.length === 0 ? (
                                                <tr>
                                                    <td colSpan={7} className="text-center py-5">
                                                        ...
                                                        <p className="text-muted fw-medium">No high-intent leads currently waiting across the platform.</p>
                                                    </td>
                                                </tr>
                                            ) : (
                                                waitingLeads.map(lead => (
                                                    <tr key={lead.id}>
                                                        ...
                                                        <td><span className="badge bg-light text-dark border">{lead.tenant}</span></td>
                                                        ...
                                                        <td className="text-end px-4">
                                                            ...
                                                            <button
                                                                ...
                                                                disabled={matchingLeadId === lead.id}
                                                                onClick={() => handleForceMatch(lead.id, lead.tenantId)}
                                                            >
                            ...
                            <div className="card-footer bg-white py-3 border-0 text-center border-top">
                                <p className="text-muted small mb-0">
                                    <i className="bi bi-robot me-1 text-primary"></i> Global AI Matching Engine Status: <span className="text-success fw-bold">Online</span>
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="modal-dialog modal-dialog-centered modal-lg">
                        <div className="modal-content border-0 shadow-lg rounded-4">
                            ...
                            <div className="modal-body p-4">
                                <div className="alert alert-info border-0 rounded-3 small mb-2">
                                    <i className="bi bi-building me-2"></i> Agency: <strong>{previewLead.tenant}</strong>
                                </div>
                            ...
                            <div className="modal-footer border-0 p-3 bg-light">
                                ...
                                <button
                                    ...
                                    onClick={() => handleForceMatch(previewLead.id, previewLead.tenantId)}
                                >
